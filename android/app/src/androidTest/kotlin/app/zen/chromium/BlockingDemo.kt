...
class BlockingDemo : DemoHarness("blocking-demo-state.json", "services-blocking-android", "blocking-demo") {
    ...
    override fun warmUp() {
        ...
        // the index; the seeded tab may have loaded before either was ready, so wait for both
        // and reload.
        ...
        while (SystemClock.uptimeMillis() < deadline &&
            !(status.getBoolean("ready") && listsHaveFilters(status) && blocking.snapshot.filterCount > 0)
        ) {
        ...
        note(
            "kotlin engine: ${blocking.snapshot.filterCount} network filters from ${blocking.snapshot.setCount} sets, " +
    ...
    private fun listsHaveFilters(status: JSONObject): Boolean {
        val lists = status.getJSONArray("lists")
        for (i in 0 until lists.length()) if (lists.getJSONObject(i).getInt("filterCount") > 0) return true
        return false
    }
     ...
     */
    private class DemoServer(private val page: String, port: Int) : Thread("blocking-demo-server") {
        private val socket = ServerSocket(port, 16, InetAddress.getLoopbackAddress())
