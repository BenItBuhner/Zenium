 ...
 *
 * The base of two drivers: [SyncDemo] (this sequence as is) and [SyncHistoryDemo] (the same
 * set-up, then Tabs from other devices and Send to your devices). Abstract, with NO default
 * arguments: a JUnit test class may have exactly one constructor, and Kotlin gives a class whose
 * parameters all have defaults a parameterless constructor besides the full one, which made
 * `SyncDemo` itself `initializationError` ("Test class can only have one constructor") when it
 * was this class with defaults (#314; the nightly sweep #332 found it). Each concrete driver
 * passes its three strings and declares nothing else about its constructor.
 */
abstract class SyncDemoBase(
    stateAsset: String,
