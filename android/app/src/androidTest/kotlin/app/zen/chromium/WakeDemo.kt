...
class WakeDemo : DemoHarness("gesture-demo-state.json", "wake", "wake-demo") {
    ...
    private val browser: MainActivity get() = activity as MainActivity

    @Test
    fun record() {
        runDemo()
        if (assertive && failures.isNotEmpty()) {
            throw AssertionError("the chrome was not painted or did not answer after: ${failures.joinToString()}")
        }
    }

    /** No cards to warm up; the seeded active page should have loaded before the baseline is taken. */
    override fun warmUp() {
        awaitPage()
        measureInsets()
    }
    ...
    override fun demo() {
        ...
        Log.i(TAG, if (failures.isEmpty()) "chrome painted after every scenario" else "chrome missing after ${failures.joinToString()}")
    }
    ...
    private fun sleepHangRenderer() {
        instrumentation.runOnMainSync {
            browser.host.chrome.evaluateJavascript(
    ...
    private fun dismissKeyguard(how: Dismiss) {
        ...
        when (how) {
            ...
            Dismiss.SWIPE -> {
                ...
                swipe(width / 2f, height * 0.82f, width / 2f, height * 0.22f, 320)
                SystemClock.sleep(1_500)
                if (keyguardShowing()) {
                    swipe(width / 2f, height * 0.82f, width / 2f, height * 0.22f, 320)
    ...
    private fun bringToFront() {
        ...
        val top = ui.rootInActiveWindow?.packageName?.toString()
        var state = ""
        instrumentation.runOnMainSync { state = browser.lifecycle.currentState.toString() }
    ...
    private fun refreshActivity() {
        ...
        if (next !== activity) {
            note("activity: recreated (${browser.lifecycle.currentState} -> new instance)")
            activity = next
        }
    }
    ...
    private fun relaunch() {
        val component = browser.componentName.flattenToString()
    ...
    private fun killRenderer(): String {
        ...
        instrumentation.runOnMainSync {
            val tab = browser.host.tabs.all().firstOrNull { it.isShown }
            (tab ?: browser.host.chrome).loadUrl("chrome://kill")
        }
    ...
    private fun check(name: String, shot: Bitmap) {
        val pill = findByLabelPrefix(PILL_LABEL)
        val bar = barInk(shot)
        ...
        instrumentation.runOnMainSync { current = browser.host.chrome }
    ...
    private fun describeViews(): String {
        ...
        instrumentation.runOnMainSync {
            ...
            text = "chrome ${chrome.width}x${chrome.height} vis=${visibilityName(chrome.visibility)} shown=${chrome.isShown} " +
                ...
                } + " resumed=${browser.lifecycle.currentState}"
        }
        return text
    }
    ...
    private fun measureInsets() {
        instrumentation.runOnMainSync {
            val bars = ViewCompat.getRootWindowInsets(browser.window.decorView)?.getInsets(WindowInsetsCompat.Type.systemBars())
            topInset = bars?.top ?: 0
    ...
    private fun swipe(fromX: Float, fromY: Float, toX: Float, toY: Float, durationMs: Long) {
        val f = Finger()
        f.down(fromX, fromY)
        f.moveBy(toX - fromX, toY - fromY, durationMs)
        f.up()
    }
