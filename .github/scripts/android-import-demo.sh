...
# (import-results.json) and its logcat lines next to the recording and the stills.
set -euo pipefail
...
grep -E "ImportDemo|TOUCH FAULT|CLAIM FAILED|closeUrlField|ImportService|TextFiles" "$DEMO_OUT/logcat.txt" | tail -n 400 > "$DEMO_OUT/import-logcat.txt" || true
