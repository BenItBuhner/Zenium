#!/usr/bin/env bash
# Runs on the workflow runner once the emulator has booted: installs the debug APK and the back
# demo driver (the BackDemo instrumentation), records the screen while the driver swipes back from
# the edge over the page, the menu and the tab overview, and collects the recording, the
# screenshots and the logs under artifacts/android-back-demo/.
#
# Handshake with the driver, through files in the app's private storage (readable via run-as):
#   files/back-demo/record     – written by the driver once its pages are loaded
#   files/back-demo/recording  – written here once screenrecord is rolling
...
sleep 3
# Gesture navigation: the back gesture is what is being recorded.
...
# Predictive back animations (a developer setting until Android 15 made them the default).
...
adb shell am instrument -w -e class app.zen.chromium.BackDemo "$runner" > "$out/instrument.txt" 2>&1 &
driver_pid=$!

ready=0
for _ in $(seq 1 1200); do
  if adb shell run-as "$app_id" test -f files/back-demo/record 2>/dev/null; then
    ready=1
    break
  fi
...
if [ "$ready" -ne 1 ]; then
  echo "::error::the back demo driver never reached the recording handshake"
...
recorder_pid=$!
sleep 1
adb shell run-as "$app_id" touch files/back-demo/recording

# Stop recording when the driver says it is done (it stays alive a little longer so the last
# frames are of the app, or of the system's back-to-home), or when it dies.
for _ in $(seq 1 720); do
  if adb shell run-as "$app_id" test -f files/back-demo/done 2>/dev/null; then
    break
  fi
  if ! kill -0 "$driver_pid" 2>/dev/null; then
    break
...
for name in $(adb shell run-as "$app_id" ls files/back-demo | tr -d '\r'); do
  case "$name" in
    *.png) adb exec-out run-as "$app_id" cat "files/back-demo/$name" > "$out/$name" ;;
  esac
done

# What the app saw of the gesture, for the record.
grep -E "ZenBack|BackDemo|OnBackInvoked|BackAnimation" "$out/logcat.txt" | tail -n 200 > "$out/back-logcat.txt" || true
