...
async function scenarioWalkthrough() {
  ...
  return runScenario('walkthrough', userData, {}, async (s, out) => {
    ...
    await s.step('accessibility', async () => {
      // The chrome's accessibility tree and axe's verdict in eight states (ci-13; the roles of
      ...
      // around it; then (a11y pass 2, W4-6) the cover behind that dialog, a toolbar control's
      // tooltip on keyboard focus, the tab rows' places and states, and the find bar's status
      // region. Each snapshot is compared with its baseline under .github/smoke/aria/
      ...
      try {
        ...
        // gone (a reset closes no tab), the find bar closed, the tab unmuted.
