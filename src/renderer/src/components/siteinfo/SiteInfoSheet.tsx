 ...
 * the track; the pane leaving is laid over it and slides out; every other pane is `hidden`. The
 * deeper of the two travels the whole width from the trailing edge OVER the other, drawn whole
 * – its own surface behind it, opacity 1 – from its first frame: what arrives is painted with
 * its content at once, and what leaves stays painted until it is off the track. The one under
 * it shifts by a third and dims to `LEVEL_UNDER_DIM` as it is covered (§11: transform and
 * opacity only). The pane leaving is out of the tree for assistive technology from its first
 * frame, the one arriving in it from its own – a focus left in the pane leaving is moved by the
 * component before the frame is read (`PhoneSheet`'s `enter`/`leave`).
 */
