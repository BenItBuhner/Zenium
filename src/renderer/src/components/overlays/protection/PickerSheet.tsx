...
/**
 * The phone's menulist (design-language-v2-draft §9.13, §10.4): the chassis `BottomSheet` – the
 * v2 sheet surface, its scrim, grip, 48 header and 44 / 64 rows are all the chassis's own –
 * portalled through the chrome layer so the frame's transform never offsets it. After the grip
 * strip the chassis header with the row's label centred (§9.16), or – when the row had a
 * description – a title block in its place (§9.23: the label 17/600 at 22, the description 15 at
 * 69% on the 22 line 4 under it). Then radio rows edge to edge with their text inset 16 (§9.25),
 * the current option checked; picking one moves the check at once, closes the sheet and then
 * applies the value, so the pane under it never changes while the sheet is still up. Escape, the
 * back gesture and the scrim close it. Never a native `<select>` and never a popover here (§9.13).
 */
      ...
      >
        ...
        <div role="radiogroup" aria-labelledby={titleId}>
          {options.map((option) => (
            <label
              key={option.value}
              className={cn('zen-sheet-item', option.description && 'zen-sheet-item-two-line')}
            >
              <input
                type="radio"
