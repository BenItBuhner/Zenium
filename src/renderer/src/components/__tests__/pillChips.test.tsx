...
describe('desktop pill (NavRow)', () => {
   ...
   * 170, the "Not secure" label under 220, and under 110 – a 130 px pill – every chip after the
   ...
   * favicon slot). happy-dom evaluates no container query, so the markers and the rule are pinned
   * here; the widths are measured on the packaged build.
   */
  it('marks every chip after the address for the narrow pill’s tier, the site icon excepted', () => {
    // Zoomed away from the default, so the zoom chip is in the pill too.
    const zoomed = tab('https://example.com/some/path', { readerable: true, zoom: 1.25 })
    const s = state(zoomed)
    ...
    const [icon, ...after] = chips
    expect(icon.classList.contains('zen-pill-chip')).toBe(false)
    for (const chip of after)
      expect(chip.classList.contains('zen-pill-chip'), chip.ariaLabel ?? undefined).toBe(true)
