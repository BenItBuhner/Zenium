...
/**
 * Accessibility › Default zoom: the value, the stepper along Chrome's zoom table, and sample
 * text at the size pages will open at (the system font size folded in when the setting says so).
 */
export function ZoomBlock({
  ...
  return (
    <div className="zen-settings-zoom-block">
      ...
      <ZoomStepper value={value} onChange={onChange} className="zen-settings-zoom-stepper" />
      <p
        className="zen-settings-zoom-preview"
        aria-hidden="true"
        style={{ fontSize: 15 * previewFactor }}
      >
...
/**
 * A value to copy (an endpoint, a token): label, the value in monospace (masked until Show when
 * secret), and the inline Copy – the one place §10.4 keeps a button in a row, because the copy
 * needs its "Copied" state.
 */
  ...
  return (
    <div className="zen-settings-copy-row">
      <div className="zen-settings-copy-text">
        ...
        <code className="zen-settings-mono">
