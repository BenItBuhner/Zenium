...
describe('the section model', () => {
  ...
  it('carries #115’s Privacy and security groups (tracking-*) at Chrome’s tracking-prevention position, behind requestBlocking', () => {
    ...
    for (const r of allRows(privacy.groups)) expect(r.id, r.label).toMatch(/^tracking-/)
    ...
    // Without the engine the category builds to nothing (`availableSections` hides it, `requires`).
