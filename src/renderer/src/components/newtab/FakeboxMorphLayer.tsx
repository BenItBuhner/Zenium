...
import { fakeboxContentWidth, widestPoseWidth } from '@renderer/lib/motion/fakebox'
 ...
 * raised panel arrives over the first half on top of the field's opaque surface, with the field's
 * words leaving, and the field's surface leaves under it over the second as the omnibox's own
 * words arrive – never two half fades stacked. The rectangle is the overview hero card's pattern
 * (`lerpRect`, left / top / width / height per frame) rather than a clip or a transform: the
 * words inside must not scale. Nor re-wrap: they are laid out once, at the widest of the field's
 * poses, and the moving box clips them, so a long placeholder never re-truncates as the width
 * passes through; the mic and the camera ride the box's right edge instead. `will-change` is on
 * the box only while a write moves it (`data-moving`, v1 §7 rule 4), never at rest on the page.
...
function FakeboxDouble(): JSX.Element {
  ...
  useLayoutEffect(() => {
    setFakeboxPainter(({ pose, state, geometry, moving }) => {
      ...
      if (content) {
        content.style.width = `${fakeboxContentWidth(widestPoseWidth(geometry), glyphs)}px`
      }
    })
  ...
  return (
    <div className="zen-fakebox-layer pointer-events-none fixed inset-0 z-[35]">
      ...
      >
        ...
        <span className="zen-fakebox-clip absolute inset-0 flex items-center overflow-hidden pl-4">
          <span
            ref={contentRef}
        ...
        {glyphs > 0 && (
          <span
            className="zen-fakebox-trailing absolute inset-y-0 right-1.5 flex items-center gap-0.5"
            aria-hidden
          >
            {voice && (
