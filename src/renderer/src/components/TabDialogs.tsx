 ...
 * autofill prompts (save / update a login, save an address or a card, choose a passkey account),
 * the address and card editors of Settings > Autofill and the vault passphrase asked for by a
 * re-authenticated command run from the chrome. The modal ones render through the
 * `FrameDialogHost` this mounts, so they centre in the box it is placed in – the content frame
 * on desktop, the shell on phones – over a scrim that dims only that box (lib/portals.tsx). The
 * star bubble is a popover: on desktop it portals to the chrome layer, anchored under the star;
 * on phones it is a sheet in the host. The zoom bubble is a desktop popover too, under the
 * pill's zoom chip. This is the frame's host: a dialog whose state lives inside the frame (a
 * phone panel's sheets, the new tab page's customise sheet) reaches it through
 * `FrameDialogPortal`.
 */
