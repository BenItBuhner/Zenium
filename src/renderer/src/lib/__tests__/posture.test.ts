...
describe('samePosture', () => {
  ...
  it('is true for the same pose and false when the kind, a side or the orientation differs', () => {
    ...
    expect(
      samePosture({ kind: 'halfOpened', hinge }, { kind: 'halfOpened', hinge: { ...hinge } })
    ).toBe(true)
    expect(samePosture({ kind: 'halfOpened', hinge }, { kind: 'flat', hinge })).toBe(false)
    expect(
      samePosture(
        { kind: 'halfOpened', hinge },
        { kind: 'halfOpened', hinge: { ...hinge, top: 401 } }
      )
    ).toBe(false)
    expect(
      samePosture(
        { kind: 'halfOpened', hinge },
        { kind: 'halfOpened', hinge: { ...hinge, orientation: 'vertical' } }
      )
    ).toBe(false)
    expect(samePosture({ kind: 'halfOpened', hinge }, { kind: 'halfOpened', hinge: null })).toBe(
      false
    )
  })
})
