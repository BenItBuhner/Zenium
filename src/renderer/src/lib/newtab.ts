 ...
 * of its card when the overview was open. With the private container the tab is a private one
 * (`tab.newPrivate`; INC-01) and the page that comes up is the private new tab page.
 */
export async function openNewTabPage(
  ...
  const tabId = await (
    options.containerId === PRIVATE_CONTAINER_ID
      ? cmd('tab.newPrivate', {})
      : cmd('tab.create', { url: BLANK_URL, active: true, containerId: options.containerId })
  ).catch(() => null)
