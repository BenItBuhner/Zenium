...
export function parseAxisTransform(value: string | null | undefined): AxisTransform | null {
  ...
  if (m3) {
    ...
    // Row-major as CSS lists it: m11 m12 m13 m14 / m21 … / m41 m42 m43 m44.
 ...
 * length is the painted one divided by the scale. A scale that is not positive and finite
...
export function layoutRectThrough(
  ...
  return {
    x: left + (painted.x - frame.x) / sx,
    y: top + (painted.y - frame.y) / sy,
    width: painted.width / sx,
    height: painted.height / sy
  }
}
