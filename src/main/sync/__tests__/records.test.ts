...
describe('diffLocal', () => {
  it('stamps new and changed records with now and keeps unchanged timestamps', () => {
    const src = sources()
    const first = diffLocal({}, collectLocal(src, defaultScope()), 1000)
    expect(first.changed).toBe(true)
    expect(first.records.every((r) => r.modified === 1000)).toBe(true)

    const again = diffLocal(first.meta, collectLocal(src, defaultScope()), 2000)
    expect(again.changed).toBe(false)
    expect(again.records.every((r) => r.modified === 1000)).toBe(true)
    ...
    expect(third.records.find((r) => r.id === src.ids.pinned.id)?.modified).toBe(1000)
  })
