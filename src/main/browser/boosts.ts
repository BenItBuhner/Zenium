...
export class BoostService {
  ...
  constructor(
    ...
    this.store = new JsonStore<Persisted>(join(userDataDir, 'zen', 'boosts.json'), 300)
  ...
  stopZap(tabId: string): void {
    ...
    this.browser.tabs.webContents(tabId)?.send('zen:zap', false)
    this.browser.state.commitVolatile()
  }
  ...
  apply(tabId: string): void {
    ...
    if (previous) {
      ...
      void wc.removeInsertedCSS(previous.key).catch(() => undefined)
    }
    const domain = getDomain(tab.url)
    ...
    if (!css) return
    void wc
      .insertCSS(css, { cssOrigin: 'user' })
      .then((key) => {
        if (this.browser.tabs.webContents(tabId) === wc) this.injected.set(tabId, { domain, key })
      })
      .catch(() => undefined)
  }
