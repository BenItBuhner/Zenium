...
export class ExtensionService {
  ...
  constructor(
    ...
    this.store = new JsonStore<Persisted>(join(userDataDir, 'zen', 'extensions.json'), 300)
  ...
  private async load(path: string): Promise<void> {
    ...
    for (const [, ses] of this.browser.sessions.persistent()) {
      try {
        const ext =
  ...
  openPopup(id: string, anchor: Rect, win: ZenWindow): void {
    ...
    const ses = this.browser.sessions.persistent()[0]?.[1]
    if (!ses) return
    ...
    const contentBounds = win.win.getContentBounds()
  ...
  closePopup(): void {
    ...
    this.popup = null
    if (win.alive) win.win.contentView.removeChildView(view)
