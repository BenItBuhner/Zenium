 ...
 * carries Chrome's sentence – "This will reset your startup page, new tab page, search engine,
 * and pinned tabs. It will also disable all extensions and clear temporary data like cookies.
 * Your bookmarks, history, and saved passwords will not be cleared." – and the reset does what
 * the sentence says and nothing else, each clause a step through the core's existing service:
 *
 ...
 *
 * Bookmarks, history, saved passwords, autofill, downloads and the site permissions are not
 * touched. Chrome's resetter also puts back the homepage, the content settings and the
 * languages, which its sentence does not name; they are left as they are here and named in the
 * W7-6 report as a follow-up ruling.
 *
 ...
 */
import type { BrowsingDataType, ExtensionInfo, Settings, Tab } from '../shared/types'
...
/** What the plan reads: the settings the sentence names, the tabs, the extensions, the region. */
export interface SettingsResetTerms {
  settings: Pick<Settings, 'startup' | 'searchEngineId' | 'searchChoice'>
...
/**
 * The reset as steps, in the order the sentence names them: always the six, each saying what
 * it does to this state – a step with nothing of its own to do (no pinned tab, no enabled
 * extension) still stands, empty, so a reader of the plan sees every clause answered.
 */
 ...
 * patch and one commit; the tabs, the extensions and the data follow. Returns the plan run.
 */
export async function resetSettings(
  ...
  for (const step of plan) {
    if (step.kind === 'startup' || step.kind === 'searchEngine') Object.assign(patch, step.patch)
  }
