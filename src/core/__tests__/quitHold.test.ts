...
import { outcome, quitChord, release, settle, start } from './quitHoldFixture'

/*
 * The quit chord held quits (session-08): the hold armed by the chord's key down, released by
 * any key up, fired at 1.5 s into a quit the hold confirmed; the panel posted to the window's
 * active page; the menu bar's Quit role refused while the press is in progress. The host and
 * the chord are `quitHoldFixture.ts`'s.
 */
