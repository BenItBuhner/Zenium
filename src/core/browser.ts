...
import { BOOKMARKS_URL, getDomain, inputToUrl } from '../shared/url'
...
export class Browser {
  ...
  private submitUrlbar(
    ...
    if (!url) return
    // The bookmark manager is a chrome surface, not a page: `zen://bookmarks` opens it in place.
    if (url === BOOKMARKS_URL) {
      this.emit('overlay.open', { kind: 'bookmarks' }, win)
      return
    }
    const routed = win.localSpace ? null : this.routeSpaceFor(url)
