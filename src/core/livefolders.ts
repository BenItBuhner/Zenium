...
export class LiveFolderService {
  ...
  constructor(private readonly browser: Browser) {
    this.store = new JsonStore<Persisted>(browser.platform.io, 'live-folders.json', 500)
  ...
  private async fetchText(url: string, headers: Record<string, string>): Promise<string> {
    ...
    try {
      ...
      if (res.text.length > LIVE_FOLDER_MAX_BYTES) throw new Error('Response larger than 1 MB')
      return res.text
    } finally {
      clearTimeout(timer)
    }
