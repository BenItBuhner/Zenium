...
export const DEFAULT_SETTINGS: Settings = {
  ...
  // `menuOrder` (the phone app menu's user order, TB-22) has no default entry: absent reads as
  // the default order (`shared/menuOrder.ts`), as does the empty list the Reset row writes – a
  // value, kept and synced, where a default entry here would put a key into every device's
  // settings record and change every record's hash at the upgrade (`core/sync/records.ts`).
